...
function timeSpanToString(startDate, endDate) {
  function num0(number, digits) {
    return number.toString().padStart(digits, '0');
  }
  let diff = endDate - startDate;
  const hour = Math.trunc(diff / (60 * 60 * 1000));
  diff -= hour * (60 * 60 * 1000);
  const min = Math.trunc(diff / (60 * 1000));
  diff -= min * (60 * 1000);
  const sec = Math.trunc(diff / 1000);
  diff -= sec * 1000;
  return `${num0(hour, 2)}:${num0(min, 2)}:${num0(sec, 2)}.${num0(diff, 3)}`;
}
...
function angleBetweenClockHands(date) {
  ...
  const angelHour = 0.5 * (60 * hour + min);
  const angelMin = 6 * min;
  let angel = Math.abs(angelHour - angelMin);
  if (angel > 180) {
    angel = 360 - angel;
  }
